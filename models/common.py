...
class BasicConv(nn.Module):
    ...
    def __init__(self, in_planes, out_planes, kernel_size, stride=1, padding=0, dilation=1, groups=1, relu=True, bn=True, bias=False):
        ...
        self.bn = nn.BatchNorm2d(out_planes,eps=1e-3, momentum=0.01, affine=True) if bn else None
...
class BasicSepConv(nn.Module):
    ...
    def __init__(self, in_planes, kernel_size, stride=1, padding=0, dilation=1, groups=1, relu=True, bn=True, bias=False):
        ...
        self.bn = nn.BatchNorm2d(in_planes,eps=1e-3, momentum=0.01, affine=True) if bn else None
        self.relu = nn.ReLU6(inplace=True) if relu else None
...
class RFB(nn.Module):
    def __init__(self, c1, c2):  
        ...
        self.start_conv = BasicConv(in_planes, in_planes, kernel_size=3, stride=1,groups=in_planes,padding=1)
        ...
        self.branch2 = nn.Sequential(
                BasicConv(in_planes, inter_planes, kernel_size=1, stride=1),
                BasicConv(inter_planes, inter_planes, kernel_size=(1,3), stride=self.stride, padding=(0,1)),
                BasicSepConv(inter_planes, kernel_size=3, stride=1, padding=2, dilation=2, relu=False)
                )
        self.branch3 = nn.Sequential(
                BasicConv(in_planes, inter_planes, kernel_size=1, stride=1),
                BasicConv(inter_planes, (inter_planes), kernel_size=(1,3), stride=1, padding=(0,1)),
                BasicConv((inter_planes), inter_planes, kernel_size=(3,1), stride=self.stride, padding=(1,0)),
